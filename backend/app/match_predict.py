...
try:
    model = joblib.load("../model/team_lr_model.pkl")
...
def get_match_features(match_ids):
    ...
    try:
        ...
        if not match_ids:
            ...
            return [0] * 10  # Нули, если нет данных
        ...
        conn.close()
        if not features:
            return get_match_features([])  # Fallback на средние
        # Усредняем признаки, преобразуя Decimal в float
...
def predict_match_outcome(blue_team, red_team):
    ...
    for team, team_name in [(blue_team, "blue"), (red_team, "red")]:
        team_features = []
        for player in team:
            game_name, tag_line, role, champion = player
            ...
            # Получаем champion_id и role_id
            champion_id = get_champion_id(champion)
            role_id = ROLES.get(role.upper(), 0)  # 0 для UNKNOWN
        ...
        # Усредняем признаки по команде (5 игроков)
    ...
    # Формируем вектор признаков: blue - red
