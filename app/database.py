...
def insert_player(game_name, tag_line):
    try:
        ...
        # Кодируем game_name и tag_line для URL
        encoded_game_name = urllib.parse.quote(game_name)
        encoded_tag_line = urllib.parse.quote(tag_line)

        # Запрос puuid
        url_account = f"https://{API_REGION}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/{encoded_game_name}/{encoded_tag_line}"
        ...
        if r.status_code != 200:
            ...
            cursor.close()
            conn.close()
            raise ValueError(f"Игрок {game_name}#{tag_line} не найден в Riot API (HTTP {r.status_code})")
        account = r.json()
        if "puuid" not in account:
            print(f"Ошибка для {game_name}#{tag_line}: нет puuid, ответ: {account}")
            cursor.close()
            conn.close()
            raise ValueError(f"Ответ API для {game_name}#{tag_line} не содержит puuid")
        puuid = account["puuid"]
        ...
        if s.status_code != 200:
            ...
            cursor.close()
            conn.close()
            raise ValueError(f"Не удалось получить данные summoner для {game_name}#{tag_line} (HTTP {s.status_code})")
        summoner = s.json()
        if "summonerLevel" not in summoner:
            print(f"Ошибка summoner для {puuid}: нет summonerLevel, ответ: {summoner}")
            cursor.close()
            conn.close()
            raise ValueError(f"Ответ API для {game_name}#{tag_line} не содержит summonerLevel")
        ...
        try:
            ...
            if player_id:
                ...
                if player_id:
                    ...
                    cursor.close()
                    conn.close()
                    raise ValueError(f"Не удалось сохранить игрока {game_name}#{tag_line} в базе данных")
